...
object TestInterpreters {
  ...
  val defaultUsers      = List(UserIds(id = defaultUserId, username = defaultUsername, externalId = defaultExternalId))
  ...
  class RemoteFileIntTest extends RemoteFileAlg[TestProgram] {
    ...
    override def remoteFileStream(fileId: RemoteFileId): TestProgram[Stream[TestProgram, Byte]] =
      wrapped(Stream.fromIterator[TestProgram]("some text".getBytes.iterator))
  }
  ...
  class FileStoreIntTest(md5Response: List[StoredFile] = List()) extends FileStoreAlg[TestProgram] {
    ...
    override def findByMd5(userId: UserId, md5: String): TestProgram[List[StoredFile]]     = wrapped(md5Response)
  ...
  class ReceiptStoreIntTest(receipts: List[ReceiptEntity] = List()) extends ReceiptStoreAlg[TestProgram] {
    override def getReceipt(userId: UserId, id: String): TestProgram[Option[ReceiptEntity]]               = wrapped(receipts.find(_.id == id))
    ...
    override def getReceipts(userId: UserId, ids: List[String]): TestProgram[List[ReceiptEntity]]         = wrapped(receipts)
    override def userReceipts(userId: UserId): TestProgram[List[ReceiptEntity]]                           = wrapped(receipts)
    override def recentUserReceipts(userId: UserId, lastModified: Long): TestProgram[List[ReceiptEntity]] = wrapped(receipts)
    override def addFileToReceipt(userId: UserId, receiptId: String, file: FileEntity): TestProgram[Unit] =
  ...
  class ImageIntTest(isImage: Boolean, imageMetaData: ImageMetaData = ImageMetaData(width = 1, height = 2, length = 3))
      extends ImageAlg[TestProgram] {
