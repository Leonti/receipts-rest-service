      ...
      entity(as[Multipart.FormData]).flatMap { formData =>
        ...
        val processedParts: Future[Seq[Future[Part]]] = formData.parts.map({ part =>
          if (part.filename.isDefined) {
            ...
            val fieldPart: Future[Part] = part.entity.toStrict(10.seconds).map(e =>
              Part(
                name = part.name,
                stringValue = Some(e.data.utf8String)))
            fieldPart
          }
        }).runWith(Sink.seq)
        ...
        val parsedFormFuture = partsFuture.map(parts => parts.foldLeft[ParsedForm](ParsedForm(Map.empty, Map.empty))((acc, part) => {
          part.file match {
