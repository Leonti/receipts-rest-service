  ...
  def fromTextAnnotation(textAnnotation: TextAnnotation): OcrTextAnnotation = {
    ...
    def toWord(word: com.google.api.services.vision.v1.model.Word): Word = {
      Word(boundingBox = toBoundingPoly(word.getBoundingBox),
        property = toTextProperty(word.getProperty), symbols = word.getSymbols.asScala.map(toSymbol))
    }
    ...
    def toParagraph(paragraph: com.google.api.services.vision.v1.model.Paragraph): Paragraph = {
      Paragraph(boundingBox = toBoundingPoly(paragraph.getBoundingBox),
        property = toTextProperty(paragraph.getProperty), words = paragraph.getWords.asScala.map(toWord))
    }

    def toBlock(b: com.google.api.services.vision.v1.model.Block): Block = {
      Block(blockType = b.getBlockType, boundingBox = toBoundingPoly(b.getBoundingBox),
        paragraphs = b.getParagraphs.asScala.map(toParagraph), property = toTextProperty(b.getProperty))
    }

    def toPage(page: com.google.api.services.vision.v1.model.Page): Page = {
      Page(height = page.getHeight, width = page.getWidth, property = toTextProperty(page.getProperty), blocks = page.getBlocks.asScala.map(toBlock))
    }
